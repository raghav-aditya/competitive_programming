...
int32_t main() {
    ...
    auto solve = [&]()->void{
		...
		Hash h1(167,A) , h2(29,A);

		if( !h2.isPallindrome() && !h1.isPallindrome() )
		{
			cout<<"YES"<<endl;
			cout<<1<<endl;
			cout<<A<<endl;
		...
		for( int i = 0 ; i < N-1 ; i++ )
		{
			if( !h1.is_substring_pallindrom(0,i) && !h2.is_substring_pallindrom(0,i) )
			{
				if( !h1.is_substring_pallindrom(i+1,N-1) && !h2.is_substring_pallindrom(i+1,N-1)){
					cout<<"YES"<<endl;
					cout<<2<<endl;
